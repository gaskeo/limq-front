    ...
    return async (dispatch: Dispatch<rootActions>) => {
        try {
            ...
            dispatch({type: FetchActionTypes.setFetch,
                payload: {identifier: 'renameUser', state: {status: 200, message: '', dataState: dataStates.requested}}})
            ...
            if (response.data['auth']) {
                ...
                dispatch({type: FetchActionTypes.setFetch,
                    payload: {identifier: 'renameUser', state: {status: 200, message: '', dataState: dataStates.received}}})
            }
        }
        catch (error: AxiosError | any) {
            dispatch({type: FetchActionTypes.setFetch,
                payload: {identifier: 'renameUser', state: {status: error.status, message:
